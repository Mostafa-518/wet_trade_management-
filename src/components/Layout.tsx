...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navigationItems = [
    ...
    { to: ROUTES.RESPONSIBILITIES, label: 'Responsibilities', icon: ClipboardList },
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={`${isSidebarOpen ? 'w-64 lg:w-80' : 'w-16'} flex-shrink-0 bg-gray-50 border-r border-gray-200 transition-all duration-300`}>
        <div className="h-full px-4 py-6 flex flex-col">
          ...
          <nav className="flex flex-col space-y-2 flex-1">
            {navigationItems.map((item) => {
              ...
              const isActive = pathname === item.to || (item.to !== ROUTES.DASHBOARD && pathname.startsWith(item.to));
              ...
              return (
                <Link
                  ...
                  className={`flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors hover:bg-gray-100 ${
                    isActive ? 'bg-gray-100 text-foreground' : 'text-muted-foreground'
                  }`}
          ...
          <div className="mt-auto">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className={`group flex items-center w-full rounded-md px-4 py-2 text-sm font-medium hover:bg-gray-100 transition-colors ${!isSidebarOpen ? 'justify-center' : ''}`}>
                  ...
                  {isSidebarOpen && (
                    <span className="ml-3 flex-1 text-left">{!isLoading ? userProfile?.full_name || 'User' : <Skeleton className="h-4 w-20" />}</span>
                  )}
                </button>
              </DropdownMenuTrigger>
