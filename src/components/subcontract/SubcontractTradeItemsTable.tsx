      ...
      <CardContent className="p-0">
        {tradeItems.length > 0 ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50">
                  ...
                  <TableHead className="text-right font-semibold">Base Amount</TableHead>
                  <TableHead className="text-right font-semibold">Wastage %</TableHead>
                ...
                {tradeItems.map((item, index) => {
                  ...
                  return (
                    <TableRow key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <TableCell className="text-right font-bold text-green-600">{formatCurrency(item.total)}</TableCell>
                    </TableRow>
                  );
                })}
                <TableRow className="border-t-2 bg-green-50">
                  <TableCell colSpan={8} className="font-bold text-lg">Total Contract Value</TableCell>
                  <TableCell className="text-right font-bold text-xl text-green-700">{formatCurrency(totalValue)}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No Trade Items</h3>
            <p className="text-gray-500">This subcontract doesn't have any trade items yet.</p>
          </div>
        )}
      </CardContent>
    </Card>
