...
export function ProjectDetailView({ projectId, onBack, onEdit }: ProjectDetailViewProps) {
  ...
  const projectSubcontracts = mockContracts.filter(contract => 
    contract.projectName?.toLowerCase().includes(project?.name.toLowerCase() || '')
  );
            ...
            <CardContent>
              {projectSubcontracts.length > 0 ? (
                <div className="space-y-4">
                  {projectSubcontracts.map(contract => (
                    <div key={contract.id} className="border rounded-lg p-4 hover:bg-muted/50">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-semibold">{contract.subcontractor}</h3>
                          <p className="text-sm text-muted-foreground">{contract.trade}</p>
                          <p className="text-sm text-muted-foreground">Value: {contract.value}</p>
                        </div>
